...
def heuristic_a_star(state):
    return (abs(state[1][1] - state[0])) * (abs(state[2][1] - state[0])) + min(abs(state[1][1] - state[0]), abs(state[2][1] - state[0]))
...
def a_star(state):
    ...
    if solution_a_star == -1:
        ...
        queue.pop(0)
        if len(queue) > 0:
            queue.sort(key=heuristic_a_star)
            a_star(queue[0])
        else:
            print("no solution")
...
def water_jug_problem(k, capacity1, capacity2):
    ...
    strategy = input("Write the strategy to be implemented:")
    if strategy.__eq__("bkt"):
        print('Backtracking')
        visited[(0, 0)] = 1
        backtracking(state)
        print(solution)
    elif strategy.__eq__("bfs"):
        print('BFS')
        visited_bfs[(0, 0)] = 1
        breadth_first_search(state)
    elif strategy.__eq__("hill-climbing"):
        print('hill_climbing')
        visited_hill[(0, 0)] = 1
        hill_climbing(state)
        if solution_hill == -1:
            print('Euristica s-a blocat intr-un maxim local, dar exista o solutie')
    elif strategy.__eq__('a star'):
        print('A star')
